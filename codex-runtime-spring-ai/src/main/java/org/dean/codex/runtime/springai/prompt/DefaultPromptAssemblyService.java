    ...
    private ResolvedPromptOutputContract resolveOutputContract(ResolvedToolContract toolContract) {
        return new ResolvedPromptOutputContract(
                ...
                List.of(
                        ...
                        "Sub-agent control tools must never target the current thread itself.",
                        "A delegated sub-agent should do its assigned work directly instead of creating another delegation layer unless the user explicitly asks for that.",
                        "After spawning a sub-agent, prefer wait_agent to observe progress instead of immediately reassigning or resuming it.",
                        "When the user asks you to continue only after several sub-agents are all finished, use wait_agent with waitForAll=true on the outstanding child threads.",
                        "When a single summarizer or final delegate should hand its result straight back to the user, use wait_agent with completeWhenDone=true for that final child.",
                        ...
                        "If user steering asks about a running sub-agent, answer from the latest known status or mailbox result instead of immediately waiting again.",
